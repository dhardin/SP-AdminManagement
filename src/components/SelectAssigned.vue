<template>
  <v-card class="flexcard" >
    ...
    <v-card-text class="grow">
      <v-data-iterator :pagination.sync="pagination" :items="sortedItems" :loading="isSearching" :search="searchAssigned" class="items" must-sort  :rows-per-page-items="[10, 20, 50, {'text':'All', 'value': -1}]" next-icon="" prev-icon="">
    ...
    ></v-progress-circular>
      ...
      <v-flex class="pl-2">
        ...
        <v-flex xs12 class="item"  slot="item" slot-scope="props">
          <transition name="slide" appear>
            <v-btn block :ripple="false" @click="selectItem(props.item, (pagination.page - 1) * pagination.rowsPerPage + props.index )" :color="props.item.selected ? 'red lighten-1' : 'red lighten-4'"  depressed light class="selectItem listItem">
              <v-container>
                <v-layout row wrap>
                  <v-flex xs1>
