...
import axios from 'axios'
  ...
  methods: {
    ...
    /*  return axios({
        url:   window.location.origin + "/_api/search/query?querytext='contentclass=STS_Site'&rowlimit='200'&selectproperties='Title,Path'",
        method: "get",
        headers: {
          "accept": "application/json;odata=verbose"
        }
    ...
    getData: function(callback, errorCallback){
      ...
      saveConfigData: function(digest, callback, errorCallback){
        ...
        if(this.config.ID > 0){
          this.$lodash.extend(headers, {
            "IF-MATCH": "*",
            "X-HTTP-Method": "MERGE"
          });
        }
        this.$lodash.extend(data, this.config, { '__metadata': { 'type': type } });
